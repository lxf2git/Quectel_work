...
sData_package init_login(char*name,char*deatAddr)
{
		...
		login_package.time[strlen(login_package.time)-1] = '\0';
		sprintf(ThisMy.dest_user,"%s",login_package.dorinform.loginInformation.userId);
		sprintf(login_package.src_user,"%s",ThisMy.dest_user);
		sprintf(login_package.dest_user,"SERVICE");
...
sData_package init_register(char*name,char*destAddr)
{
		...
		login_package.time[strlen(login_package.time)-1] = '\0';
		sprintf(login_package.dest_user,"SERVICE");
		sprintf(login_package.src_user,"%s",login_package.dorinform.loginInformation.userId);
		return login_package; 
}
sData_package init_addfriend(char*name,char *destAddr)
{
		...
		sprintf(data_package.src_user,"%s",ThisMy.dest_user);
		do{
				printf("init\n");
				printf(GRE"you want addfriend name:"NONE);
				fflush(stdout);
				bzero(buf,sizeof(buf));
				scanf("%s",buf);
				printf(RED"please again:"NONE);
				fflush(stdout);
				scanf("%s",data_package.dest_user);
				
		}while((strcmp(data_package.dest_user,buf) != 0) &&(printf("两次输入不一致\n")) );
		time_t mytime;
		mytime = time(NULL);
		sprintf(data_package.time,"%s",ctime(&mytime));
		data_package.time[strlen(data_package.time)-1] = '\0';
