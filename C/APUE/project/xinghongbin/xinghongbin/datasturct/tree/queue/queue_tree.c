...
void mid_tree(pTree pRoot,pQueue pqueue);
void show_tree(pTree pRoot,pQueue pqueue);
...
//		push(pStack,pRoot);
		...
		while(pop(pqueue,&pTemp) != -1)
		{
				...
				if(pTemp->plc != NULL)
				{
						printf("(%c->plc %c push) ",pTemp->data,pTemp->plc->data);
						push(pqueue,pTemp->plc);
				}
				if(pTemp->prc != NULL)
				{
						printf("(%c->prc %c push)",pTemp->data,pTemp->prc->data);
						push(pqueue,pTemp->prc);
				}
				printf("\n");
...
void show_tree(pTree pRoot,pQueue pqueue)
{
//	prev_tree(pRoot,pqueue);
//	printf("\n");
	mid_tree(pRoot,pqueue);
	printf("\n");
//	last_tree(pRoot,pqueue);
//	printf("\n");
}
/*
